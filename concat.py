...
dir_path = "/isis/home/huynhc1/Paper-Text-Scraper/datatables-processed"
...
#print(res)
def concat(f1):
    f1 = "datatables-processed/"+f1
    df = pd.read_csv(f1, delimiter=",", index_col=False)
    df1 = pd.read_csv("complete2.csv",delimiter=",",index_col=False)
    frames = [df, df1]
    result = pd.concat(frames, ignore_index=True)
    result.drop_duplicates()
    print("DF size: " + str(result.shape[0]))
    result.to_csv("complete2.csv", index=False, mode='w')
...
u2 = df[['Task', 'Dataset']].value_counts().reset_index(name='count')
u1 = u1[['Task', 'Dataset']]
u2 = u2[['Task', 'Dataset']]
